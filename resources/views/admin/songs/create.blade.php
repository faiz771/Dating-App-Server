...
@section('content')
    ...
    <div class="row layout-top-spacing" id="cancel-row">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            <div class="statbox widget box box-shadow">
                <div class="widget-header">
                    <form action="{{ url('/save-song') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-row">
                         ...
                         <div class="form-group col-md-6">
                                <label for="pkgName">Song Title</label>
                                <input type="text" name="name" class="form-control" id="songTitle"
                                    placeholder="Song Title" value="{{ old('name') }}">
                            </div> 
                                <div class="form-group col-md-6">
                                  <label for="inputAddress2">Song Artists</label>
                                    <select class="form-control tagging" name="type">
                                        <option hidden disabled selected>--- Select Artist Type ---</option>
                                        @foreach($artist as $artist_type)
                                            <option value="{{ $artist_type->name }}">{{ $artist_type->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            <div class="form-group col-md-6">
                                <label for="inputAddress">Song Price</label>
                                <input oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"type = "number" maxlength = "25" class="form-control" id="inputAddress" name="price"
                                    placeholder="Song Price" value="{{ old('price') }}">
                            </div>
                        </div>

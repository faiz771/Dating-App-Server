@extends('layouts.app', ['title' => 'Orders', 'cat_name' => 'EIN', 'page_name' => 'manage_orders'])
                    ...
                    <div class="row">
                          ...
                          <form method="POST" action="{{url('/order_filter_bystate')}}" id="formformeal" enctype="multipart/form-data">
                              ...
                              <div class="input-group">
                                <select class="form-control" name="status">
                                  ...
                                  <input type="hidden" name="service" value="EIN"> 
                                <button type="submit" class="btn btn-success input-group-addon" style="border-radius: inherit;">State</button>
                              </div>
                           </form>
                      </div>
                        ...
                        <div class="col-xl-3 col-md-3 col-sm-3 col-3 d-flex justify-content-end">
                            <form method="POST" action="{{url('/order_filter_byStatus')}}" id="formformeal" enctype="multipart/form-data">
                                @csrf
                                <div class="input-group">
                                <input type="hidden" name="ser" value="EIN">
                                  <select class="form-control" name="status">
                                      <option value="all"  @if(!empty($status)) {{ $status == 'all'  ? 'selected' : ''}} @endif>All</option>
                ...
                <table id="alter_pagination" class="table table-hover" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach ($orders as $key => $val)
                            ...
                            @if($val->service_type == 'EIN')
                            <tr>
                                <td> {{ $key + 1 }} </td>
                                <td> @if(!empty($val->user->customer_id)) {{ $val->user->customer_id }}@endif</td>
                                <td> @if(!empty($val->user->f_state)) {{ $val->user->f_state }} @endif</td>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="{{ url('/ein_edit-order/' . $id) }}" class="text-success" title="Edit"><i
        ...
        <form action="{{url('/assign_coupon')}}" method="POST">
          ...
          <div class="modal-body">
            <div class="form-group">
                ...
                <input type="hidden" class="form-control" name="type" id="" aria-describedby="" value="EIN">
